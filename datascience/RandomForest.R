...
training_data <- read.csv('F:\\R_CBAP\\codes\\analytics\\datascience\\Train_Churn.csv')

testing_data <- read.csv('F:\\R_CBAP\\codes\\analytics\\datascience\\Test_Churn.csv')
...
for (i in 1:5){
  ...
  #applying chisquare test ---- output is churn for selecting features(column) of categorial data
  #removing all feature so that all non contributing is removed
  print(chisq.test(table(training_data$Churn, train_categorical[,i])))
}
#result col(state) - p(.002) value(state) has less than 0.05 so significant 
...
cor_matrix
#reformating the data for row column structure
...
# conclusion - we can drop either minutes or carghes as both give same information hghly corelated
...
outlier_removal <- function(dataset, columns){
  ...
  for (col in columns){
    ...
    #filer data which is outside outlier range. The outliers are removed from data --- ! - remove all outliers
...
#Scaling the data - cnverting data to z score
...
#do same treatment for testing dataset
...
#all IV are converted to z scores & DV are in form of 0&1
cleaned_test_data
...
# ********************************Random Forest********************************
# any number in bracket R will keep log of it so that it will repeat the same steps everytime
#otherwise second time you will run data will be same
set.seed(1)
...
#-----------------------------test dataset to check learning-----------------------------------------------------------
...
autoplot(curves, 'PRC') #92% good more the curve good it is
