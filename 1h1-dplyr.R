...
filter(mtcars, gear == 4 & mpg < 25 | cyl == 8)
...
filter(mtcars, row_number() == 1L)  #first row
head(mtcars)
filter(mtcars, row_number() == n()) #last row
filter(mtcars, between(row_number(), 5, n()-2)) #between rows



#mutate----
mutate(mtcars, displ_l = disp / 61.0237)#keeps other new col (create new salary column by multiplying existing value)
mutate(mtcars, new_mpg = mpg + 5)
transmute(mtcars, displ_l = disp / 61.0237) #removes other cols 
mutate(mtcars, cyl = NULL) #do not display particular column(cyl)
...
slice(mtcars, n())
slice(mtcars, 5:n())
slice(mtcars,seq(1,32,2))   #gives alternate rows
...
mtcars %>% group_by(gear) %>% summarise(mean(wt), n=n())
mtcars %>% group_by(gear) %>% summarise(meanWt = mean(wt), n=n()) %>% mutate(newMean =meanWt*2)

mtcars %>% group_by(gear) %>% summarise(mean(mpg), mean(hp), max(disp))
...
mtcars %>% group_by(gear, cyl, carb) %>% summarise_all(funs(mean(mpg), max(wt)))

mtcars %>% sample_n(5)
mtcars %>% sample_frac(.3)

mtcars %>% arrange(am,mpg) #sort by am mpg
mtcars %>% select(mpg,am) %>% arrange(mpg)  %% head(5)
